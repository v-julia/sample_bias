...
input_dir = "D:/DATA/samplebias/biases/EV-A71_alignments/skygrids_logs"
input_dir = "D:/DATA/samplebias/biases/EV-A71_alignments/lengths"
...
models = data.frame(c('random_single','random_groups', '3', '5', '2_1'),
                    c('random single', 'random groups', '3% identity threshold', '5% identity threshold', 
                      'max=2 seq, 1%'),
                    stringsAsFactors = F)

models = data.frame(c('random_single', '3', '5', '2_1'),
                    c('random single', '3% identity threshold', '5% identity threshold', 
                      'max=2 seq, 1%'),
                    stringsAsFactors = F)
models = data.frame(c('200'),
                    c('200 sequences'),
                    stringsAsFactors = F)
...
df_heights$model_f = factor(df_heights$model, levels=c('random single', '3% identity threshold', '5% identity threshold', 
                                                       'max=2 seq, 1%'))
df_rates$model_f = factor(df_rates$model, levels=c('random single', '3% identity threshold', '5% identity threshold', 
                                                   'max=2 seq, 1%'))

df_rates$median = as.numeric(df_rates$median)
df_heights$median = as.numeric(df_heights$median)
df_rates
df_heights

r = ggplot(df_rates) + geom_point(aes(x=as.numeric(df_rates$x),y=as.numeric(df_rates$median))) + 
  ...
  facet_grid(~ model_f) + labs(y="mean Rate", x="") +
...
h = ggplot(df_heights) + geom_point(aes(x=as.numeric(df_heights$x),y=as.numeric(df_heights$median))) + 
  ...
  facet_grid(~ model_f) + labs(y="treeHeight", x="")  +
  theme_classic() +
  theme(panel.spacing=unit(1, "lines"),
      panel.border = element_rect(color = "black", fill = NA, size = 0.1), 
      strip.background = element_rect(color = "black", size = 0.1),
...
do.call(grid.arrange,r,h)


r_boxplots = ggplot(df_rates, aes(x=model, y=median)) + 
...
h_boxplots = ggplot(df_heights, aes(x=model, y=median)) + 
   geom_boxplot()+ theme_classic() +
  theme(panel.spacing=unit(1, "lines"),
        panel.border = element_rect(color = "black", fill = NA, size = 0.1), 
        strip.background = element_rect(color = "black", size = 0.1),
...
grid.arrange(r_boxplots,h_boxplots)
