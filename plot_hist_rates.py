...
def plot_hist_rates(rates, mu, sigma, title, filename):
    ...
    min_xlim = min(mu-3*sigma, min(rates)) - step
    max_xlim = max(mu+3*sigma,max(rates)) + step
...
def get_rates(f_name, output_dir):
    ...
    # etmplate for output name
    out_name_temp = os.path.splitext(os.path.split(f_name)[1])[0]
    file_out = open(output_dir + out_name_temp + '_rates.txt', 'w')
...
if __name__ == "__main__":
    ...
    parser.add_argument("-out_dir", "--output_dir", type=str,
                        help="Output directory to save generated figures")
    ...
    if args.output_dir == None:
        args.output_dir = os.path.split(args.input_file)[0]

    rates, mu, sigma, rates_log, mu_log, sigma_log = get_rates(args.input_file, args.output_dir)

    # template for figure name
    fig_name_temp = os.path.splitext(os.path.split(args.input_file)[1])[0]

    # plots histograms of rates and log rates
    out_fig_name = Path(args.output_dir,fig_name_temp).with_suffix('_rates.' + args.format)
    plot_hist_rates(rates, mu, sigma,
            "Rate distribution in tree of " + args.title, out_fig_name)
    out_fig_name = Path(args.output_dir,fig_name_temp).with_suffix('_lrates.' + args.format)
    plot_hist_rates(rates_log, mu_log, sigma_log,
            "Log rate distribution in tree of " + args.title, out_fig_name)
